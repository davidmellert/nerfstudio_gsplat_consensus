...
# ============================================================
# Render Step 2 result + run Step 3 (DC regularization)
# Run with: nohup bash scripts/run_finetune_only_2.sh > finetune2.log 2>&1 &
# ============================================================
...
# ----------------------------------------------------------
# Render Step 2 (normal fine-tuning, already trained)
# ----------------------------------------------------------
NORMAL_CONFIG=$(ls -t outputs/bicycle/splatfacto-gaussian-batch-colmap/*/config.yml 2>/dev/null | head -1)
if [ -z "$NORMAL_CONFIG" ]; then
  echo "ERROR: No Step 2 config found"; exit 1
fi
echo ">>> Rendering normal fine-tuning from: $NORMAL_CONFIG"
mkdir -p renders
